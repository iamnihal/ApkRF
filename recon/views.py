    ...
    if request.method == "POST":
        if "apkfile" in request.FILES:
            ...
            if (
                ...
                or file_content_type == "application/vnd.android.package-archive"
            ):
...
def analyze_apk(request):
    if os.path.isfile(MEDIA_URL + file_name + ".apk"):
        apkleaks = subprocess.run(
            [
                "python",
                tools_dir + "/apkleaks/apkleaks.py",
                "-f",
                MEDIA_URL + file_name + ".apk",
                "-o",
                f"{output_dir}/{file_name}.txt",
            ],
            ...
            for item in itemList:
                if item == section:
                    return render(
                        request,
                        "recon/result.html",
                        {"resultContext": itemList, "title": section},
                    )
    return render(
        request, "recon/result.html", {"resultContext": "Something is Wrong!!"}
    )
...
def download_result(request):
    if request.method == "GET":
        ...
        with open(result_file, "r") as fh:
            response = HttpResponse(fh.read(), content_type="text/plain charset=utf-8")
